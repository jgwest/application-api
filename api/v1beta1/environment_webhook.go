...
func (r *Environment) validateEnvironment() error {
	...
	if unstableConfig != nil {
		...
		if unstableConfig.KubernetesClusterCredentials.APIURL != "" {
			if _, err := url.ParseRequestURI(unstableConfig.KubernetesClusterCredentials.APIURL); err != nil {
				return fmt.Errorf(err.Error() + ": API URL must be an absolute URL starting with an 'https' scheme ")
			}
		}
	}
